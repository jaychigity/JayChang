import type { Metadata } from 'next'
import BreadcrumbSchema from '@/components/BreadcrumbSchema'
import Link from 'next/link'
import CashFlowPlanner from './CashFlowPlanner'

export const metadata: Metadata = {
  title: 'Free Cash Flow Planner | Year-by-Year Retirement Projection',
  description:
    'Free year-by-year cash flow projection. Model income, Social Security, portfolio withdrawals, expenses, taxes, and one-time goals from now through retirement. See net flows and portfolio balance for every year. No sign-up required.',
  alternates: { canonical: 'https://www.advisorjay.com/tools/cash-flow-planner' },
  openGraph: {
    title: 'Free Cash Flow Planner',
    description:
      'See your year-by-year income, expenses, and portfolio balance from today through retirement. Model salary, Social Security, pension, withdrawals, taxes, and major goals.',
    url: 'https://www.advisorjay.com/tools/cash-flow-planner',
  },
}

export default function CashFlowPlannerPage() {
  return (
    <>
      <BreadcrumbSchema
        items={[
          { name: 'Tools', href: '/tools' },
          { name: 'Cash Flow Planner', href: '/tools/cash-flow-planner' },
        ]}
      />

      {/* Hero Banner */}
      <section className="bg-[#333333] pt-[100px] pb-[48px] px-[20px] md:px-[40px] lg:px-[80px]">
        <div className="max-w-[960px] mx-auto text-center">
          <p className="font-sans text-[12px] font-semibold uppercase tracking-[0.15em] text-[#1d7682] mb-4">
            RETIREMENT PLANNING TOOL
          </p>
          <h1 className="font-serif text-[32px] md:text-[48px] font-light text-[#F7F4EE] leading-tight">
            Cash Flow Planner
          </h1>
          <p className="font-sans text-[16px] md:text-[18px] text-[#F7F4EE]/70 mt-4 max-w-[640px] mx-auto leading-relaxed">
            See every dollar coming in and going out, year by year, from today through the end of
            your plan. Income, withdrawals, Social Security, taxes, goals, and portfolio balance in
            one table.
          </p>
          <p className="font-sans text-[14px] md:text-[15px] text-[#F7F4EE]/50 mt-3 max-w-[600px] mx-auto leading-relaxed">
            Most people have never seen their full financial picture laid out this way. This is
            where clarity starts.
          </p>
          <p
            className="font-sans"
            style={{ fontSize: 13, fontWeight: 600, color: '#F7F4EE', marginTop: 12, letterSpacing: '0.05em' }}
          >
            No sign-up required &middot; Instant results
          </p>
          <div className="mt-6 flex justify-center">
            <Link
              href="/tools"
              className="font-sans text-[13px] text-[#F7F4EE]/50 hover:text-[#1d7682] transition-colors"
            >
              &larr; All Tools &amp; Calculators
            </Link>
          </div>
        </div>
      </section>

      <div className="h-[1px] bg-[#1d7682]/20" />

      {/* Planner */}
      <section className="bg-[#F7F4EE]">
        <CashFlowPlanner />
      </section>

      {/* Related Tools */}
      <section style={{ backgroundColor: '#FAFAF8', padding: '48px 40px' }}>
        <div style={{ maxWidth: 760, margin: '0 auto', textAlign: 'center' }}>
          <h3
            className="font-serif"
            style={{ fontSize: 24, fontWeight: 400, color: '#333333', marginBottom: 16 }}
          >
            Related Tools
          </h3>
          <div
            className="font-sans"
            style={{
              display: 'flex',
              flexWrap: 'wrap',
              justifyContent: 'center',
              gap: '12px 24px',
              fontSize: 15,
            }}
          >
            <a href="/tools/monte-carlo-simulator" style={{ color: '#1d7682', textDecoration: 'underline' }}>
              Monte Carlo Portfolio Simulator
            </a>
            <a href="/tools/retirement-savings-calculator" style={{ color: '#1d7682', textDecoration: 'underline' }}>
              Retirement Readiness Calculator
            </a>
            <a href="/tools/social-security-calculator" style={{ color: '#1d7682', textDecoration: 'underline' }}>
              Social Security Calculator
            </a>
            <a href="/tools/rmd-calculator" style={{ color: '#1d7682', textDecoration: 'underline' }}>
              RMD Calculator
            </a>
          </div>
        </div>
      </section>
    </>
  )
}
